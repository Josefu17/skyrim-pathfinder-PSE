...
async function Init() {
    ...
    try {
        ...
        await fetch('../project_information/Doku.md')
            ...
            .then(text => document.getElementById('documentation').innerHTML = marked.parse(text))
            .then(() => {set_anchor_targets();});
...
function set_anchor_targets() {
    ...
    for (let reference of references) {
        ...
        let ref_str = reference.href.replace(/https:\/\/group2.proxy.devops-pse.users.h-da.cloud\/doc.html/g, '');
...
function replaceSpecialChars(inputString) {
    return inputString
        ...
        .replace(/ /g, '-')  // Ersetzt Leerzeichen durch '-'
        .replace(/[^\w\-]/g, '-')  // Ersetzt alle nicht-alphanumerischen Zeichen (außer Bindestrich) durch '-'
        .toLowerCase();
}
