...
def fetch_and_store_map_data_if_needed():
    ...
    try:
        ...
        for city in data["cities"]:
            db_city = CityDAO.get_city_by_name(city["name"])
            if not db_city:
                db_city = City(
                    name=city["name"],
        ...
        if new_connections:
            ConnectionDAO.save_connections_bulk(new_connections)
